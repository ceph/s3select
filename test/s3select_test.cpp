...
{
    s3select s3select_syntax;
    const std::string input_query = "select substr(trim(both from '   foobar   '),2,3) from stdin ;" ;
    auto status = s3select_syntax.parse_query(input_query.c_str());
    ASSERT_EQ(status, 0);
    s3selectEngine::csv_object s3_csv_object(&s3select_syntax);
    std::string s3select_result;
...
{
    s3select s3select_syntax;
    const std::string input_query = "select substr(trim(both '12' from '1112211foobar22211122'),1,6) from stdin ;" ;
    auto status = s3select_syntax.parse_query(input_query.c_str());
    ASSERT_EQ(status, 0);
    s3selectEngine::csv_object s3_csv_object(&s3select_syntax);
    std::string s3select_result;
